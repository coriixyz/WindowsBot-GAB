	...
	if(suffix) {
			...
			if(member.user.bot || [msg.author.id, bot.user.id].indexOf(member.id) > -1 || bot.getUserBotAdmin(msg.channel.guild, serverDocument, member) > 0) {
				msg.channel.createMessage({
					embed: {
                        ...
                        color: 0xFF0000,
						description: `Sorry, I can't strike **@${bot.getName(msg.channel.guild, serverDocument, member)}** for some reason ✋`
					}
				});
			} else {
				...
				member.user.getDMChannel().then(ch => {
					ch.createMessage({
						embed: {
                            ...
                            color: 0xFF0000,
							title: `You just got a strike from **@${bot.getName(msg.channel.guild, serverDocument, msg.channel.guild.members.get(msg.author.id), true)}** on **${msg.channel.guild.name}**${reason ? ":" : ""}`,
					...
					msg.channel.createMessage({
						embed: {
                            ...
                            color: 0x00FF00,
							description: `Ok, **@${bot.getName(msg.channel.guild, serverDocument, member)}** now has ${targetMemberDocument.strikes.length} strike${targetMemberDocument.strikes.length == 1 ? "" : "s"} 🚦 I striked them via PM ⚠`
						}
					});
					ModLog.create(msg.channel.guild, serverDocument, "Strike", member, msg.member, reason);
				}).catch();
			}
		} else {
			msg.channel.createMessage({
		...
		msg.channel.createMessage({
			embed: {
                ...
                color: 0xFF0000,
                description: "Who do you want me to strike? 😮 Why should I strike them?"
			}
		});
	}
};
