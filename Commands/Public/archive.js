module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(!suffix || isNaN(suffix)) {
		...
		msg.channel.createMessage(`${msg.author.mention} 🔢 Please provide an amount to fetch!`);
	} else {
		...
		doArchive(num > 100 ? 100 : num, msg.channel.lastMessageID, (err, archive) => {
			if(err) {
				...
				msg.channel.createMessage("🛑 Permission Error: Make sure I have permissions to read message history!");
			} else {
				msg.channel.createMessage("Here you go! ✅", {
					...
					msg.channel.createMessage("😓 Please use a smaller archive size!");
				});
			}
		});
	}
